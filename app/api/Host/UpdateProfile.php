<?php
$app->post('/api/host/UpdateProfile',function($request){
    ...
    if ($request->hasHeader('token')) {
        ...
        if($valid==true)
        {
            $userID = getHostIDByToken($token,2);
            ...
            $query = "SELECT ProfileImage from host WHERE HostID = $userID";
            ...
            if($profile_image!=""){
                ...
                if($data["ProfileImage"]==""){
                    $query = "UPDATE host SET ProfileImage='$imageToDBURL', FirstName = '$first_name', LastName = '$last_name', Contact = '$contact', Pitch = '".mysqli_real_escape_string($mysqli,$pitch)."' WHERE HostID = $userID";
                }
                else {
                    $query = "UPDATE host SET FirstName = '$first_name', LastName = '$last_name', Contact = '$contact', Pitch = '".mysqli_real_escape_string($mysqli,$pitch)."' WHERE HostID = $userID";
                }
            } else {
                 $query = "UPDATE host SET FirstName = '$first_name', LastName = '$last_name', Contact = '$contact', Pitch = '".mysqli_real_escape_string($mysqli,$pitch)."' WHERE HostID = $userID"; 
            }
            ...
            if (mysqli_query($mysqli, $query)) {
                ...
                //update language
                if(isset($language)){
                    $sqlDeleteLanguage = "DELETE FROM userlanguage WHERE UserID = $userID AND UserType=2";
                    if (mysqli_query($mysqli, $sqlDeleteLanguage)) {
                        $sql =null;
                        //echo $language;
                        for($i=0;$i<COUNT($language);$i++){
                            $sql.= "INSERT INTO userlanguage (Language, UserID, UserType) VALUES ('$language[$i]', $userID, 2);";
                            //echo $language[$i];
                        }
                        //echo $sql;
